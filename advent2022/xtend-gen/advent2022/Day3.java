...
public class Day3 {
  public static void main(final String[] args) {
    ...
    final Function2<Integer, String, Integer> _function = (Integer acc, String e) -> {
      int _xblockexpression = (int) 0;
      {
        ...
        int _value = Day3.toValue((IterableExtensions.<Character>head(IterableExtensions.<Character>filter(((Iterable<Character>)Conversions.doWrapArray(part1)), _function_1))).charValue());
    ...
    final Function2<Pair<ArrayList<char[]>, Integer>, String, Pair<ArrayList<char[]>, Integer>> _function_1 = (Pair<ArrayList<char[]>, Integer> acc, String e) -> {
      Pair<ArrayList<char[]>, Integer> _xblockexpression = null;
      {
        ...
        if (_equals) {
          ...
          {
            final Function1<Character, Boolean> _function_2 = (Character it) -> {
              return Boolean.valueOf(ArrayExtensions.contains(acc.getKey().get(1), (it).charValue()));
            };
            final Function1<Character, Boolean> _function_3 = (Character it) -> {
              return Boolean.valueOf(ArrayExtensions.contains(acc.getKey().get(2), (it).charValue()));
            };
            final Character id = IterableExtensions.<Character>head(IterableExtensions.<Character>filter(IterableExtensions.<Character>filter(((Iterable<Character>)Conversions.doWrapArray(acc.getKey().get(0))), _function_2), _function_3));
            ArrayList<char[]> _newArrayList_1 = CollectionLiterals.<char[]>newArrayList();
            Integer _value = acc.getValue();
            int _value_1 = Day3.toValue((id).charValue());
            int _plus = ((_value).intValue() + _value_1);
            _xblockexpression_1 = Pair.<ArrayList<char[]>, Integer>of(_newArrayList_1, Integer.valueOf(_plus));
          }
