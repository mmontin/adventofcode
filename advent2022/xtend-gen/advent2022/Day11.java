...
public class Day11 {
  public static class Monkey {
    ...
    public void play() {
      ...
      final Consumer<Integer> _function = (Integer it) -> {
        this.inspected++;
        long current_item = (this.items.remove(0)).longValue();
        Long _xifexpression = null;
        boolean _isPresent = this.adjustment.isPresent();
        if (_isPresent) {
          _xifexpression = this.adjustment.get();
        } else {
        ...
        if (((current_item % this.divisibility) == 0)) {
          _xifexpression_1 = this.ifTrue;
        } else {
          _xifexpression_1 = this.ifFalse;
        }
    ...
    public void playSmart() {
      ...
      final Consumer<Integer> _function = (Integer it) -> {
        ...
        final Consumer<Map.Entry<Integer, Long>> _function_1 = (Map.Entry<Integer, Long> entry) -> {
          Long _xifexpression = null;
          boolean _isPresent = this.adjustment.isPresent();
          if (_isPresent) {
            _xifexpression = this.adjustment.get();
        ...
        new HashSet<Map.Entry<Integer, Long>>(_entrySet).forEach(_function_1);
        int _xifexpression = (int) 0;
        Long _get = current_item.get(Integer.valueOf(this.divisibility));
        boolean _equals = ((_get).longValue() == 0);
        if (_equals) {
          _xifexpression = this.ifTrue;
        } else {
          _xifexpression = this.ifFalse;
        }
    ...
    public ArrayList<ArrayList<HashMap<Integer, Long>>> apply() {
      ArrayList<ArrayList<HashMap<Integer, Long>>> _xblockexpression = null;
      {
        ...
        final Consumer<Integer> _function = (Integer i) -> {
          ...
          final Consumer<Long> _function_1 = (Long item) -> {
            final HashMap<Integer, Long> newCongruences = CollectionLiterals.<Integer, Long>newHashMap();
            newOne.add(newCongruences);
            final Function1<Day11.Monkey, Integer> _function_2 = (Day11.Monkey it) -> {
              return Integer.valueOf(it.divisibility);
            };
            final Consumer<Integer> _function_3 = (Integer div) -> {
              newCongruences.put(div, Long.valueOf(((item).longValue() % (div).intValue())));
            };
            ListExtensions.<Day11.Monkey, Integer>map(Day11.monkeys, _function_2).forEach(_function_3);
          };
          currentMonkey.items.forEach(_function_1);
        };
        new IntegerRange(0, 7).forEach(_function);
        _xblockexpression = output;
      }
  ...
  public static void main(final String[] args) {
    final Consumer<Integer> _function = (Integer it) -> {
      final Consumer<Day11.Monkey> _function_1 = (Day11.Monkey it_1) -> {
        it_1.play();
      };
      Day11.monkeys.forEach(_function_1);
    };
    new ExclusiveRange(0, 20, true).forEach(_function);
    final Function1<Day11.Monkey, Long> _function_1 = (Day11.Monkey it) -> {
    ...
    final Consumer<Day11.Monkey> _function_2 = (Day11.Monkey it) -> {
      it.inspected = 0;
    };
    Day11.monkeys.forEach(_function_2);
    final Consumer<Integer> _function_3 = (Integer it) -> {
      final Consumer<Day11.Monkey> _function_4 = (Day11.Monkey it_1) -> {
        it_1.playSmart();
      };
      Day11.monkeys.forEach(_function_4);
    };
    new ExclusiveRange(0, 10000, true).forEach(_function_3);
    final Function1<Day11.Monkey, Long> _function_4 = (Day11.Monkey it) -> {
      return Long.valueOf((-it.inspected));
    };
