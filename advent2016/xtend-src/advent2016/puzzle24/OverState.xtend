...
class OverState implements State {
	...
	override neighbours() {
		if (toVisit.equals(newHashSet(Launcher.initial)))
			newArrayList(
				new Pair(new OverState(Launcher.initial, newHashSet) as State,
					Launcher.distances.get(newHashSet(currentPosition, Launcher.initial))))
		else
			toVisit.filter[!equals(Launcher.initial)].map [
				val newToVisit = new HashSet(toVisit)
				newToVisit.remove(it)
				new OverState(it, newToVisit) -> Launcher.distances.get(newHashSet(it, currentPosition))
			]
	}
